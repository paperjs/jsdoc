 ...
 * @singleton
 * New plugin manager singleton.  This plugin mgr is event-based.  when a plugin is registered to it,
 * the plugin should provide a list of events it wishes to register.  when PluginMgr sees that word, it'll
 * check to see if it has any listeners registered for it -- if so, it'll fire the event.
 *  
 * @author Chris Scott
 * 
 */
JSDOC.resistor.PluginMgr = function() {
    ...
    return {
         ...
         * with the corresponding handler to call.
         * eg: 
         * {
         *     commentsrc : function(param) { print("I want to know when a comment is created"); },
         *     Class.create : function(param) { print ("I want to when the NAME Class.create is found in token-stream"); }
         * }   
        ...
        register : function(plugin, events) {                           
            for (var key in events) {
                var event = key.replace(/\./, '').toLowerCase();
